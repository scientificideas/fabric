...
func (oc *OrdererClient) Broadcast() (BroadcastOrdererClient, error) {
	return newMulticastBroadcastClient(oc)
}
...
func newMulticastBroadcastClient(oc *OrdererClient) (*multicastBroadcastClient, error) {
	addresses := []string{oc.address}
	if strings.Contains(oc.address, ",") {
		addresses = strings.Split(oc.address, ",")
	}

	mbc := &multicastBroadcastClient{}

	for _, addr := range addresses {
		conn, err := oc.CommonClient.clientConfig.Dial(addr)
		if err != nil {
			return nil, errors.WithMessagef(err, "orderer client failed to connect to %s", oc.address)
		}
